```{post} 2023-10-12
:tags: noticia, organización, evento
:category: wiki
:language: Español
:excerpt: 2
```

[English](/wiki/evento-2024-01-12-guayacamp-en.md) | [Español](/wiki/evento-2024-01-12-guayacamp.md) <!-- l10n:select -->


#  GuayaCamp 2024 🚀

```{image} ./evento-2024-01-12-guayacamp.md-data/banner.png
:alt: banner
:class: bg-transparent
:align: center
```



## 👋 Saludos internauta!

El `13 de enero de 2024` celebraremos en GuayaHack nuestra primera integración! GuayaCamp será un día de {ref}`conferencias <wiki/evento-2024-01-12-guayacamp.md#agenda>` sobre tecnología, programación, aprendizaje, la vida profesional y más; éste tendrá lugar en [Medellín](https://maps.app.goo.gl/dhQeyBgsQPeg5oVm6), Colombia. 

## Beep Bop !^$% GuayaHack 

Esta página contiene toda la información esencial sobre GuayaCamp;(otros detalles se discutirán en el [Issue de Planeación](https://gitlab.com/guayahack/main/-/issues/117)). En caso de dudas o sugerencias, escríbenos por [Discord](https://discord.gg/RHePucN4e9) o a `info@guayahack.co`.

### ¡ [Regístrate](https://forms.gle/YLkTwczr4zgpbVUr8) y conecta con la comunidad !

A fin de poder planear con exactitud y garantizar un espacio agradable, es de suma importancia que todos los asistentes se registren oportunamente.

La fecha límite de registro y aportes monetarios para los amantes de la pizza es `Diciembre 1, 2023`. Igualmenente será posible registrarse y aportar el día del eveneto toda vez que haya espacio para más participantes.

```{image} /_static/guayhackito-pink.png
:alt: GuayHackito
:class: bg-transparent
:width: 11em
:align: left
```

```{image} /_static/guayhackito-pirate.png
:alt: GuayHackito
:class: bg-transparent
:width: 10em
:align: right
```

<p align="center" style="font-size: 2rem;">
  <a href="https://forms.gle/YLkTwczr4zgpbVUr8" style="font-size: 2rem; white-space: nowrap;">👉 Registro 👈</a>
</p>

```{image} /_static/guayhackito-sombrero-flying-yellow.png
:alt: GuayHackito
:class: bg-transparent
:width: 10em
:align: center
```

%```{image} /_static/guayhackito-sombrero.png
%:alt: GuayHackito
%:class: bg-transparent
%:width: 11em
%:align: right
%```






## ¿Cúando, Dónde y Cuánto?

```{image} ./evento-2024-01-12-guayacamp.md-data/treehouse.png
:alt: treehouse
:class: bg-transparent
:width: 12em
:align: left
```

📅 `GuayaCamp 2024` tendrá lugar de forma híbrida durante el Sábado `2024-01‐13` y la integración de los moderadores voluntarios será en El Retiro, Colombia al día siguiente.

📍 El ciclo de [desconferencias](https://es.wikipedia.org/wiki/Desconferencia) se llevará a cabo en [POR DEFINIR](https://maps.app.goo.gl/4YTXGcRULUHSLJw78) y la integración de los moderadores en [Finca Las Cordilleras](https://www.instagram.com/lascordilleras/).

📹 Grabaciones del evento, cuando las haya, estarán disponibles através de 🔗 [Zoom](https://zoom.us) y 🔗 [Youtube](https://www.youtube.com/@guayahack)

💰 El acceso al evento `no tiene costo`, pero requiere [registrarse](https://forms.gle/YLkTwczr4zgpbVUr8). Para organizar un evento del agrado de todos, contribuciones voluntarias de `50,000 | 100,000 | 200,000` COP son bienvenidas e incluyen pizza 🍕 y una bebida 🍹.

## Agenda

Este evento ha sido agregado a nuestro {doc}`/calendario` principal 

La agenda correspondiente al [calendario propio del evento ](https://calendar.google.com/calendar/u/0/embed?src=cc1058b39f9c45d8031a5e5aa56e617ade17d58445a6667af420612f54007f16@group.calendar.google.com&ctz=America/Bogota&dates=20240112%2F20240114) también está embebido directamente aquí 👇

<div class="responsiveCal">
<iframe src="https://calendar.google.com/calendar/embed?height=600&wkst=2&dates=20240112%2F20240114&bgcolor=%2385ff54&ctz=America%2FBogota&mode=AGENDA&showPrint=1&showNav=0&showTitle=0&showDate=0&showTabs=0&showCalendars=0&src=Y2MxMDU4YjM5ZjljNDVkODAzMWE1ZTVhYTU2ZTYxN2FkZTE3ZDU4NDQ1YTY2NjdhZjQyMDYxMmY1NDAwN2YxNkBncm91cC5jYWxlbmRhci5nb29nbGUuY29t&color=%23F6BF26" style="border:solid 1px #777; filter: invert(.9) saturate(1.2) hue-rotate(145deg);" width="800" height="600" frameborder="0" scrolling="no"></iframe>
</div>


## Organizadores 

{doc}`@jdsalaro </community/member/jdsalaro/index>`, {doc}`@DavidQP </community/member/davidqp/index>`, {doc}`@harold </community/member/harold/index>`, {doc}`@davidadarme </community/member/davidadarme/index>`, {doc}`@camilo.java </community/member/camilo.java/index>`.


```{image} ./evento-2024-01-12-guayacamp.md-data/kite.png
:alt: futuristic kite
:class: bg-transparent
:align: center
:height: 12em
```

## Detalles para los {doc}`Moderadores </wiki/organizacion-rol-moderador>`

### Logística y otros

Este es el [issue de sondeo](https://gitlab.com/guayahack/main/-/issues/84) anterior en donde decidimos organizar y llevar a cabo GuayaCamp: nuestro espacio y ciclo de conferencias para socializar y aprender entre miembros e interesados de la comunidad.

#### Llegada

Los moderadores y voluntarios llegarán a Medellín entre el 11 o 12 de enero de 2024.

##### Hospedaje

###### Moderadores y voluntarios

Los moderadores y voluntarios se hospedaran en 📍 [Finca Las Cordilleras](https://www.instagram.com/lascordilleras/).

###### Participantes

Participantes y miembros de la comunidad podrán solicitar unirse para pasar la noche de Enero 13 y la mañana de Enero 14 con los moderadores en el campo 🌳

#### Partida

La mayoría de los moderadores y voluntarios partirán de Medellín durante el 14 o 15 de Enero.

#### Transporte

El transporte de los moderadores y voluntarios desde Medellín y hasta 📍 [Finca Las Cordilleras](https://www.instagram.com/lascordilleras/) será llevado a cabo por GuayaHack.

%## Agradecimientos
%
%`#todo:write`
%


### Términos y condiciones del Guayacamp

1. **Pago y presupuesto**:
   - Las contribuciones se recibirán a través de una cuenta bancaria designada o plataformas de pago en línea, como Nequi, Paypal, etc y su uso comunicado con transparencia a la comunidad.
   - Se mantendrá una bitácora de gastos a fin de realizar buen uso de los recursos recaudados

2. **Alojamiento**:
   - Para moderadores y voluntarios, compartir con previo aviso las fechas de llegada y partida
   - Aquellos que reciben hospedaje y apoyo de sostenimiento tratar de tener responsabilidad y ser flexibles con la logística del GuayaCamp

3. **Transporte**:
   - Acordar puntos de encuentro entre participantes para mayor eficiencia en el transporte de los participantes.

4. **Actividades y horarios**:
   - Ser flexibles para adaptarse a los intereses de todos en el desarrollo de actividades o excursiones juntos.
   - Consultar la agenda del evento y mostrar puntualidad para evitar retrasos innecesarios.

5. **Emergencias y contacto**:
   - Intercambiar números de teléfono y datos de contacto de emergencia.

6. **Política de cancelación**:
    - Avisar con anterioridad para cancelación para alojamiento o participación en la actividad

7. **Código de conducta y resolución de conflictos**:
   - Crear un entorno amigable y parchadito depende de todos
   - Respetar el espacio personal de los demás, poner atención la limpieza de todos los espacios utilizados al igual que al nivel de ruido.
   - Abordar conflictos y desacuerdos de manera respetuosa y amigable, priorizando la armonía del grupo.
   - Está estrictamente prohibido el uso, posesión, distribución o compra de sustancias psicoactivas y estupefacientes en las instalaciones de GuayaCamp.
   - En GuayaHack nos reservamos el derecho de prohibirle la participación a miembros o externos quienes atenten en contra de éste codigo de conducta y los términos del evento

8. **Cumplimiento de las leyes locales**:
    - Recordar a todos que deben respetar las leyes y regulaciones locales durante el viaje.

Es importante que todos los participantes del GuayaCamp estén de acuerdo con estos términos y condiciones antes de comenzar el viaje. La comunicación abierta y el respeto mutuo son clave para que el viaje sea exitoso y agradable para todos.

